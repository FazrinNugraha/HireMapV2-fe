...
import { SpatialLeafletMap, getSpatialStats, getSweetSpotColor, getSweetSpotLabel, getSweetSpotScore } from '../components/map/SpatialLeafletMap'
...
export function SpatialPage({ metadata, prediction }: SpatialPageProps) {
  ...
  const [selectedLocation, setSelectedLocation] = useState(prediction?.lokasi ?? '')
  ...
  useEffect(() => {
    getSpatialSummary()
      ...
      .catch((err: unknown) => {
        setError(err instanceof Error ? err.message : 'Gagal mengambil data spasial.')
      })
  }, [])
  ...
  const selectedSummaryItem = useMemo(
    () => summary.find((item) => item.Lokasi_Clean === selectedLocation) ?? summary[0],
            ...
            {isDetailLoading ? (
              <div className="flex flex-1 flex-col gap-4">
                ...
                <div className="mt-4 flex flex-col gap-3">
                  {[1, 2, 3].map((i) => (
                    <div key={i} className="flex justify-between border-b border-[#E5E2E0] pb-3">
                      <div className="h-3 w-1/3 rounded-full shimmer" />
              ...
              <div key={locationDetail.lokasi} className="flex flex-1 flex-col animate-fade-slide-down h-full">
                <LocationDetailCard
              ...
              <p className="text-sm text-[#696969]">Pilih lokasi di peta atau dropdown.</p>
            )}
          </div>
        </aside>
      </section>
          ...
          <table className="w-full border-collapse text-left">
            <thead>
              <tr className="bg-[#F3F0EE] text-xs font-bold uppercase tracking-wider text-[#696969]">
                <th className="border-b border-[#E5E2E0] py-4 pl-8 pr-4 text-center w-16">Rank</th>
                <th className="border-b border-[#E5E2E0] p-4">Kota</th>
                <th className="border-b border-[#E5E2E0] p-4">Lowongan</th>
                <th className="border-b border-[#E5E2E0] p-4">Est. Kos / bulan</th>
                <th className="border-b border-[#E5E2E0] py-4 pl-4 pr-8">Job Opportunity</th>
              </tr>
            </thead>
  ...
  return (
    <>
      ...
      <div className="flex flex-col gap-0">
        ...
        />
        {detail.category_jobs !== null && selectedCategory !== 'All Industries' && (
          <SidebarMetric
        ...
        <SidebarMetric
          ...
          value={selectedCategory === 'All Industries' ? 'Semua Industri' : selectedCategory}
        />
      </div>
