...
function getScoreGrade(score: number): { label: string; color: string; bg: string; description: string } {
  if (score >= 80) return { label: 'Sangat Layak', color: '#16a34a', bg: '#dcfce7', description: 'Kondisi karir, tempat tinggal, dan jarak sangat mendukung untuk posisi ini.' }
  if (score >= 65) return { label: 'Layak', color: '#2563eb', bg: '#dbeafe', description: 'Cukup seimbang secara finansial dan operasional dengan sedikit penyesuaian.' }
  if (score >= 50) return { label: 'Cukup', color: '#ca8a04', bg: '#fef9c3', description: 'Ada beberapa faktor (seperti jarak jauh atau pengeluaran kos tinggi) yang perlu dipertimbangkan.' }
  if (score >= 35) return { label: 'Perlu Pertimbangan', color: '#dc2626', bg: '#fee2e2', description: 'Kondisi kurang ideal — pertimbangkan jarak commute harian atau penawaran gaji.' }
  return { label: 'Berisiko Tinggi', color: '#7f1d1d', bg: '#fef2f2', description: 'Banyak faktor tidak mendukung. Disarankan mencari alternatif yang lebih dekat atau bergaji lebih tinggi.' }
}

function buildAuditPrompt(prediction: SalaryPredictionResponse, score: number, distance: number, domicile: string): string {
  return (
...
export function FeasibilityScoreCard({ prediction, spatialSummary, onRequestAudit }: FeasibilityScoreCardProps) {
  ...
  const distance = useMemo(() => {
    ...
    return calculateDistance(originItem.lat, originItem.lon, destItem.lat, destItem.lon)
  }, [destItem, originItem])
  ...
  const dimensions: ScoreDimension[] = useMemo(() => {
    return [
      ...
      },
      {
        ...
        statusColor: commuteScore >= 80 ? '#16a34a' : commuteScore >= 50 ? '#ca8a04' : '#dc2626',
      },
      {
        ...
        statusColor: confidenceScore >= 80 ? '#16a34a' : confidenceScore >= 60 ? '#ca8a04' : '#dc2626',
      },
    ]
  }, [affordScore, ratio, prediction.estimasi_kos, competeScore, mult, commuteScore, distance, domicile, prediction.lokasi, confidenceScore, confidence])
  ...
  // Chart.js Doughnut data untuk Total DSS Score
  const donutData = {
    datasets: [
      {
    ...
    <div className="overflow-hidden rounded-[32px] bg-white shadow-[0_4px_20px_rgba(0,0,0,0.06)]">
      ...
      <div className="border-b border-[#E4E2DC] px-7 pt-7 pb-6">
        ...
        <p className="mt-1 text-sm text-[#696969]">
          Dihasilkan secara dinamis berdasarkan: hunian, kualifikasi profil, jarak tempuh commute, dan reliabilitas data.
        </p>
      </div>
      ...
      <div className="p-7">
        ...
        <div className="mt-6">
          <div className="flex flex-col gap-2 rounded-2xl bg-[#F9F8F6] p-4 border border-[#E4E2DC] transition-all hover:border-[#A0A09A]">
            <div className="flex items-center justify-between gap-3">
              <label htmlFor="domicile-select" className="text-xs font-bold uppercase tracking-wider text-[#696969]">
                📍 Domisili Anda (Asal)
              </label>
              <select
                id="domicile-select"
            ...
            <p className="text-[11px] text-[#A0A09A]">
              Pilih domisili saat ini untuk menghitung jarak & kelayakan commute ke {prediction.lokasi}.
            </p>
          </div>
        </div>
        ...
        <div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2">
          {dimensions.map((dim) => {
            // Stacked Horizontal Bar Data to represent progress bar
            const miniBarData = {
              labels: [''],
              datasets: [
                {
              ...
              >
                ...
                <div className="flex items-center justify-between gap-3">
                  <div className="min-w-0">
                    <h4 className="text-sm font-bold text-[#141413] truncate">{dim.label}</h4>
                  ...
                  <span 
                    ...
                    style={{ backgroundColor: `${dim.statusColor}18`, color: dim.statusColor }}
                  >
                    {dim.rawScore}/100
                  </span>
                </div>
        ...
        <p className="mt-6 text-[11px] leading-5 text-[#A0A09A] border-t border-[#E4E2DC] pt-4">
          ⚡ Skor dihitung dinamis menggunakan model spasial terintegrasi (formula Haversine untuk commute, regresi housing untuk kos, koreksi ML untuk gaji, dan probabilitas ambiguitas data).
        </p>
      </div>
    </div>
  )
