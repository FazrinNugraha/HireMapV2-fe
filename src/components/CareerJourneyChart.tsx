...
function buildJourneySteps(prediction: SalaryPredictionResponse): JourneyStep[] {
  ...
  const salaries = META.map((m) => Math.round(referenceBase * MULTIPLIERS[m.key]))

  return META.map((m, i) => {
    let active = matchActive(m.key)
    if (!hasMatch) {
      const closest = Object.entries(MULTIPLIERS).reduce((acc, [k, v]) =>
        Math.abs(v - prediction.m_pengalaman) < Math.abs(MULTIPLIERS[acc] - prediction.m_pengalaman) ? k : acc,
        'fresh'
      )
      active = m.key === closest
    }
    const prev = i > 0 ? salaries[i - 1] : null
    const growthPct = prev ? parseFloat((((salaries[i] - prev) / prev) * 100).toFixed(1)) : null
...
export function CareerJourneyChart({ prediction }: { prediction: SalaryPredictionResponse }) {
  ...
  const totalGrowth = (((steps[3].salary - steps[0].salary) / steps[0].salary) * 100).toFixed(1)
  ...
  const chartOptions: ChartOptions<'line'> = {
      ...
      x: {
        ...
        ticks: {
          ...
          font: {
            ...
            size: 12,
            weight: (ctx: { index: number }) => ctx.index === activeIndex ? 'bold' : 'normal',
          },
        },
      },
      y: {
    ...
    plugins: {
      ...
      tooltip: {
        ...
        cornerRadius: 14,
        displayColors: false,
        titleFont: { family: "'Sofia Sans', Arial, sans-serif", size: 11, weight: 'bold' },
        bodyFont: { family: "'Sofia Sans', Arial, sans-serif", size: 14, weight: 'bold' },
        callbacks: {
          title: (items: TooltipItem<'line'>[]) => steps[items[0].dataIndex].sublabel,
          label: (item: TooltipItem<'line'>) => `  ${formatRupiah(item.raw as number)}`,
        },
      },
    },
  }
      ...
      <div className="border-b border-[#E4E2DC] px-8 pt-7 pb-5">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2 rounded-full border border-[#E4E2DC] px-4 py-2">
            ...
            <span className="text-xs font-semibold text-[#696969]">Estimasi Gaji</span>
          </div>
        </div>
        ...
        <div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-4">
          {steps.map((step) => (
            ...
            >
              ...
              <span className="text-[11px] text-[#B0ADA7]">{step.sublabel}</span>
            </div>
          ))}
        </div>
      </div>
