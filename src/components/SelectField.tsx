...
export function SelectField({
  ...
  // Menutup dropdown saat user klik area di luar komponen.
  useEffect(() => {
  ...
  return (
    <div
      ref={containerRef}
      className={`relative ${variant === "form" ? "grid gap-1.5" : "inline-block"} min-w-0`}
    >
      {label && variant === "form" && (
        ...
        {isOpen && (
          <div
            className={`absolute z-[2000] mt-1.5 max-h-60 overflow-y-auto rounded-[24px] border border-[#E5E2E0] bg-white p-1.5 shadow-[0_12px_40px_rgba(0,0,0,0.08)] animate-fade-slide-down ${
              variant === "form"
                ? "w-full left-0"
                : "left-0 right-0 min-w-full sm:left-auto sm:min-w-[220px]"
            }`}
          >
            {options.map((option) => {
