  ...
  return (
    <section className="flex h-[calc(100vh-130px)] min-h-[700px] flex-col overflow-hidden rounded-[32px] bg-white shadow-[0_8px_30px_rgba(0,0,0,0.08)] md:col-span-8">
      ...
      <div className="border-b border-[#E4E2DC] bg-white p-6 md:p-7">
        ...
        <h2 className="mt-3 text-2xl font-semibold tracking-[-0.02em] text-[#141413]">
          AI Career Consultant
        </h2>
        <p className="mt-1.5 text-sm leading-6 text-[#696969]">
          Tanyakan soal negosiasi, strategi interview, skill growth, atau keputusan hunian.
        </p>
  ...
  return (
    <div className="flex max-w-[85%] gap-3">
      ...
      <div className="flex items-center gap-1.5 rounded-2xl rounded-tl-sm bg-[#E4E2DC] px-4 py-3">
        ...
        <span className="h-2 w-2 animate-bounce rounded-full bg-[#696969]" style={{ animationDelay: '150ms' }} />
        <span className="h-2 w-2 animate-bounce rounded-full bg-[#696969]" style={{ animationDelay: '300ms' }} />
      </div>
    </div>
  )
}
    ...
    <div className="space-y-1">
      {lines.map((line, index) => {
        ...
        if (currentLine.startsWith('####')) {
          return (
            <h4 key={index} className="mt-3 mb-1 text-sm font-bold text-[#141413]">
        ...
        if (currentLine.startsWith('###')) {
          return (
            <h3 key={index} className="mt-4 mb-1 text-base font-bold text-[#141413]">
              {renderInlineMarkdown(currentLine.replace(/^###\s*/, ''))}
            </h3>
          )
        }
        if (currentLine.startsWith('##')) {
          return (
            <h2 key={index} className="mt-4 mb-2 text-lg font-bold text-[#141413]">
              {renderInlineMarkdown(currentLine.replace(/^##\s*/, ''))}
            </h2>
          )
        }
        ...
        if (currentLine.startsWith('* ') || currentLine.startsWith('- ')) {
          ...
          return (
            <li key={index} className="ml-4 list-disc text-sm text-[#262627] mb-1">
